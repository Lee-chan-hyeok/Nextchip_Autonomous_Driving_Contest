...
def train_and_save(name, ep, save_period, batch, result_dir, exist_ok, data):
    model_path = f'/content/files/yaml_files/{name}'
    if not ('.yaml' in name or '.pt' in name):
        print('너 바보, .yaml .pt 둘 다 아님')
        return

    model = YOLO(model_path)
    model.train(data=rf'/content/colab/cfg/colab_{data}.yaml', exist_ok=exist_ok, epochs=ep, save_period=save_period, batch=batch, project=result_dir, name=name.split('.')[0])
